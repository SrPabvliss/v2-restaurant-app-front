...
const ClientsView = () => {
	const { customers, loadCustomers, customersLoaded, setCustomersLoaded, setCustomers } =
		useCustomerStore();
						...
						<ModalContent>
							{(onClose) => (
								<>
									...
									<ModalBody >
										<p >
											¿Está seguro que desea eliminar el cliente {selectedCustomer?.firstName} {selectedCustomer?.firstLastName}?
										</p>
									</ModalBody>
									<ModalFooter>
										<Button
										...
										<Button
											...
											onClick={async () => {
												...
												const result = await deleteCustomer(selectedCustomer?.id.toString() as string);
												if (result) {
													setCustomers(customers?.filter((customer) => customer.id !== selectedCustomer?.id) as ICustomer[]);
